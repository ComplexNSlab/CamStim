...
import yaml
    ...
    """
    Simplified base experiment: no psychopy, photodiode, or monitor info.
    Loads wait time and experiment name from config_files/baseExperiment.yaml.
    ...
    def __init__(self, experiment_id, mouse_id, daq=None, log_dir="./logs", config_path=None, *args, **kwargs):
        ...
        self.config_path = config_path or os.path.join(os.path.dirname(__file__), '../config_files/baseExperiment.yaml')
        # Load config
        with open(self.config_path, 'r') as f:
            config = yaml.safe_load(f)
        self.experiment_name = config.get('name', 'baseExperiment')
